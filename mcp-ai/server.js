...
function getYoutubeAudioUrl(videoId) {
  return new Promise((resolve, reject) => {
    if (!videoId) return reject(new Error('Missing videoId'));
    ...
    const child = execFile(
      ...
      (error, stdout, stderr) => {
        ...
        if (!line) return reject(new Error('yt-dlp did not return a URL'));
        resolve(line.trim());
      }
    );
...
app.get('/api/youtube/stream_meta', async (req, res) => {
  try {
    ...
    res.json({
      videoId,
      watch_url: watchUrl,
      audio_url: `/youtube/audio?id=${encodeURIComponent(videoId)}`,
      direct_audio_url: audioUrl,
      source: 'youtube',
    });
  } catch (e) {
    console.error('/api/youtube/stream_meta error', e?.message || e);
    res.status(500).json({ error: e?.message || 'Không lấy được stream YouTube' });
  }
});
...
app.get('/youtube/audio', async (req, res) => {
  try {
    ...
    if (!videoId) return res.status(400).json({ error: 'Missing id (videoId)' });
    ...
    console.error('/youtube/audio error', e?.message || e);
    res.status(502).json({ error: e?.message || 'Không stream được audio YouTube' });
  }
});
...
app.get('/api/youtube/search', async (req, res) => {
  try {
    ...
    const maxResults = Math.min(
      15,
      Math.max(1, Number.isFinite(Number(req.query.maxResults)) ? Number(req.query.maxResults) : 5)
    );
    if (!q) return res.status(400).json({ error: 'Missing q' });

    // First call: search to get basic items and videoIds
    ...
    res.json({ query: q, maxResults, count: videos.length, videos });
  } catch (e) {
    console.error('/api/youtube/search error', e?.message || e);
    res.status(500).json({ error: e?.message || 'Không tìm kiếm được YouTube' });
  }
});
