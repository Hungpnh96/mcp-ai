...
npm start
```
Mặc định server chạy tại: `http://localhost:8002`

## UI kiểm thử
Mở trình duyệt tới: `http://localhost:8002/`
...
```bash
curl "http://localhost:8002/api/song?id=ZOACFBBU"
```
- Phát trực tiếp (trình duyệt):
```
http://localhost:8002/api/song/stream?id=ZOACFBBU
```
- Tìm kiếm:
```bash
curl "http://localhost:8002/api/search?q=l%E1%BA%A1c%20tr%C3%B4i"
```
